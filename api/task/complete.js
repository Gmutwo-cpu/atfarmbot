...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const reward = reward_coins || 50;
    await supabase.from('completed_tasks').insert([{ telegram_id, task_code, reward_coins: reward }]);
    await supabase.from('users').update({ coins: (user.coins || 0) + reward }).eq('telegram_id', telegram_id);
    ...
    return res.status(200).json({ success: true, message: `Task completed! Reward +${reward} Coins`, user: updatedUser, completed_tasks });
  } catch (err) {
